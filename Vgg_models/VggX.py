...
"""
  reference:https://github.com/Horizon2333/imagenet-autoencoder/tree/main/models
"""
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, layers, enable_bn=False):
        ...
        if layers == 1:
            ...
            # self.add_module('EncoderLayer0', layer)
        else:
            ...
            for i in range(layers):
                ...
                self.add_module('{} EncoderLayer'.format(i), layer)
                # self.add_module('EncoderLayer{}'.format(i), layer)

        maxpool = nn.MaxPool2d(kernel_size=2, stride=2)

        self.add_module('{} MaxPooling'.format(layers), maxpool)
        # self.add_module('MaxPooling{}'.format(layers), maxpool)
    def forward(self, x):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, layers, enable_bn=False):
        ...
        if layers == 1:
            ...
            for i in range(layers):
                ...
                self.add_module('{} DecoderLayer'.format(i + 1), layer)
                # self.add_module('DecoderLayer{}'.format(i + 1), layer)
